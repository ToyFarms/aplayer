...
void fft_process(audio_analyzer *analyzer, audio_callback_param p)
{
    ...
    memmove(ctx->in, ctx->in + nb_samples,
            (ctx->in_size - nb_samples) * sizeof(ctx->in[0]));

    for (int i = 0; i < nb_samples; i++)
        ctx->in[ctx->in_size - nb_samples + i] = p.out[i * p.nb_channels];

    for (int i = 0; i < ctx->out_size; i++)
        ctx->freqs[i] = sqrtf((float)(ctx->out[i][0] * ctx->out[i][0] +
                                      ctx->out[i][1] * ctx->out[i][1]));

    analyzer->callback(&(analyzer_fft_ctx){.freqs = ctx->freqs,
                                           .size = ctx->out_size,
...
#define IS_POWER_OF_TWO(x) (((x) & ((x) - 1)) == 0)
    ...
    ctx->freqs = malloc(ctx->out_size * sizeof(float));
    assert(ctx->freqs != NULL);

    ctx->plan =
        fftw_plan_dft_r2c_1d(ctx->in_size, ctx->in, ctx->out, FFTW_ESTIMATE);
